...
db_loc = 'C:\\sqlite\\db\\navis_test_one_results_table.db'
...
def test_table():
    '''populate table with test names & id'''
...
def get_tests():
    for item in tests:
        read_xml_func.tests('clashgroup', item[0], path + '\\' + item[0] + ' ' + item[1] + '.xml', results_date)
        read_xml_func.tests('clashresult', item[0], path + '\\' + item[0] + ' ' + item[1] + '.xml', results_date)
